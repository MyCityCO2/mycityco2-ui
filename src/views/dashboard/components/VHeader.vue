...
<template>
  <div class="flex h-16 shrink-0 justify-between items-center lg:items-start gap-x-4 bg-white px-4">
    <h3>Les émissions de GES à {{ cityStore.currentCity.name }}</h3>
    ...
    <div class="block lg:hidden">
      <button class="bg-gray-50 p-3 rounded-xl group" @click="cityStore.openSelector()">
        <MagnifyingGlassIcon
          class="h-5 w-5 text-gray-900 text-opacity-40 group-hover:text-emerald-500"
          aria-hidden="true"
        />
      </button>
    </div>
